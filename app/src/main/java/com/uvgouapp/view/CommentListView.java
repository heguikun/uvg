...
import com.uvgouapp.application.UWEApplication;
...
public class CommentListView extends LinearLayout {
    ...
    private View getView(final int position) {
        ...
        builder.append(setClickableSpan(bean.contentUserName, bean.contentUser));

        if (!TextUtils.isEmpty(bean.replyUserName)) {
            builder.append(" 回复 ");
            builder.append(setClickableSpan(bean.replyUserName, bean.replyUser));
        }
        builder.append(": ");
        //转换表情字符
        builder.append(EncodeUtils.urlDecode(bean.content, "UTF-8"));
        //展示评论回复内容
    ...
    private SpannableString setClickableSpan(final String textStr, final String id) {
        ...
        subjectSpanText.setSpan(new SpannableClickable(itemColor) {
                                    ...
                                    public void onClick(@Nullable View widget) {
                                        //---------------进入个人详情页----------------
                                        Intent intent = new Intent(UWEApplication.getInstance(), CircleInformationActivity.class);
                                        intent.putExtra("userId", id);
                                        UWEApplication.getInstance().startActivity(intent);
                                    }
