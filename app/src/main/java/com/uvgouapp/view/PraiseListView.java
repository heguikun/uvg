...
public class PraiseListView extends TextView {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        try {
            //textview的默认颜色
            itemColor = typedArray.getColor(R.styleable.PraiseListView_item_color, getResources().getColor(R.color.color_5868a2));
    ...
    public void notifyDataSetChanged() {
        ...
        if (datas != null && datas.size() > 0) {
            ...
            for (int i = 0; i < datas.size(); i++) {
                item = datas.get(i);
                if (item != null) {
                    builder.append(setClickableSpan(item.thumbsName, i));
                    if (i != datas.size() - 1) {
                        builder.append(", ");
                    }
                }
    ...
    private SpannableString setImageSpan() {
        ...
        imgSpanText.setSpan(new ImageSpan(UWEApplication.getInstance(), R.drawable.item_give, DynamicDrawableSpan.ALIGN_BASELINE),
